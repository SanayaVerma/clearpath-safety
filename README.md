...
- **AI Integration:** I integrated the **Google Gemini 1.5 Flash** model via the `google-genai` SDK. Rather than just acting as a chatbot, the AI acts as a data-processor. It receives the raw JSON response from the government database and uses a custom prompt to perform "text-simplification" and "urgency classification." 
- **The AI Logic:** The core of the solution is the prompt engineering that instructs Gemini to identify the specific danger (e.g., fire risk vs. cosmetic issue) and provide a concrete next step for the user. I chose Gemini for its high speed, efficiency in processing technical text, and accessible developer tier.

## 3. Challenges & Innovation
One major challenge was handling the volume of data. Some older car models have dozens of recalls. I had to design the UI using "expanders" so the user isn't overwhelmed. Innovation-wise, most recall tools just give you a link to a PDF. ClearPath is unique because it interprets that content in real-time using generative AI, providing immediate clarity without requiring the user to do extra research.

## 4. Responsible AI & Accuracy
To ensure responsible use, the app includes a clear disclaimer that AI advice is for informational purposes. To verify accuracy, I tested the outputs against known major recalls (like the Takata Airbag recall) to ensure Gemini did not downplay high-risk situations. By using a "Grounding" technique—where the AI is strictly limited to summarizing the provided government text—I significantly reduced the risk of AI "hallucinations."

## 5. Reflections & Learning
Building ClearPath Safety deepened my understanding of how APIs work and the importance of "Human-Centric AI." I learned that AI is most powerful when it acts as a bridge between high-level data and everyday people. This project taught me how to manage API keys securely using Streamlit Secrets and how to design a tool that is truly accessible to my community using modern models like Gemini.